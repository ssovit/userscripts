...
(function() {
    ...
    if ($("#frmvoucher")
        ...
        $("#applicationid,#applicationid1").val("TSC0011");
        $("#voucherid,#voucherid1").val("2075-5");
        $("#applicationid")
            .on('keyup', function() {
            $('#applicationid1')
                .val($(this)
        ...
        var _voucherResubmit=$("<a></a>").text("Resubmit").css({
            position:'fixed',top:"100px",right:0,
            background:"#000",
            color:"#FFF",
            padding:20,display:'block',borderRadius:"5px 0 0 5px"});
        $('body').append(_voucherResubmit);
        _voucherResubmit.on('click',function(e){
            e.preventDefault();
            $("#btnvoucher").attr("disabled",false);
            $("#btnvoucher").trigger('click');
