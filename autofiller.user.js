...
(function() {
    ...
    if($('#frmdform').length>0){
        //$('head').append($("<script></script>").attr({type:"text/javascript",src:"https://raw.githubusercontent.com/Shuvayatra/preeti/master/preeti.js"}));
        var _textarea=$("<textarea></textarea>");
        $('#frmdform').prepend(_textarea.css({width:"100%",height:"300",fontFamily:"Preeti",fontSize:24}));
        var processIt=function(_text){
        ...
        _textarea.on('keyup',function(){
            processIt(preeti($(this).val()));
        });


        $('#inserttalim').on('change',function(){
            $('#talimdinesanstha').val($('#passedfrom').val());
            $('#talimkoname').val($('#passedexam').val());
            $('#talimutirnasal,#talimsamma').val($('#passeddate').val());
            $('#talimshreni').val($('#passedshreni').val());
            $('#talimbisaye').val($('#passedmainsubject').val());
        });
    }
